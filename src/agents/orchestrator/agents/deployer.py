"""Deployer agent — triggers HTTP deployment."""
...
class DeployerAgent(BaseAgent):
    """Procedural agent: trigger deployment via HTTP (test or production).

    Delegates to the existing exec_release_deployer handler to preserve exact
    ...
    async def run(self, job, workspace, ctx) -> AgentResult:
        """Trigger deployment and optionally poll for completion."""
        ...
        await execute_release_deployer(handler_ctx, job, provider, workspace)
