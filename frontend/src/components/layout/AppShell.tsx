...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // Text label opacity — fades in/out with sidebar
  const labelCls = `transition-opacity duration-200 ${isOpen ? 'opacity-100' : 'opacity-0'}`

  return (
    <div className="flex h-screen bg-gray-950">
      {/* Sidebar — pinned (locked open) by default. Users can unpin to get hover-expand behavior. */}
      <aside
      ...
      >
        <div className="w-60 h-full flex flex-col">
          ...
          <div className="h-[53px] flex items-center gap-3 px-4 border-b border-gray-900">
            ...
            <span className={`text-sm font-semibold text-white tracking-wide whitespace-nowrap ${labelCls}`}>
              Agent Platform
            </span>
            {/* Pin/unpin button */}
            <button
          ...
          <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
            ...
            <div className="pt-5 pb-1.5 px-3 flex items-center justify-between">
              <span className={`text-[11px] font-medium text-gray-600 uppercase tracking-widest whitespace-nowrap ${labelCls}`}>
                Projects
              </span>
              <button
              ...
              return (
                <div key={p.id} className="group flex items-center">
                  ...
                  >
                    ...
                    />
                    <span className={`truncate ${labelCls}`}>{p.name}</span>
                  </Link>
                  <Link
                    to={`/projects/${p.id}/chat`}
                    className={`hidden group-hover:flex items-center px-1.5 transition-colors ${
                  ...
                  <Link
                    ...
                    className="hidden group-hover:flex items-center px-1.5 text-gray-600 hover:text-gray-400 transition-colors"
          ...
          <div className="px-2 py-3 border-t border-gray-900">
            <div className="flex items-center gap-3 px-3 whitespace-nowrap">
              ...
              <span className={`text-sm text-gray-500 truncate flex-1 ${labelCls}`}>{user?.display_name}</span>
              <button
                onClick={logout}
                className={`text-xs text-gray-600 hover:text-gray-400 transition-colors shrink-0 ${labelCls}`}
              >
                Sign out
              </button>
            </div>
