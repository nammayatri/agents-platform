...
import { X, PanelBottomClose, PanelBottomOpen, PanelLeftClose, PanelLeftOpen, Loader2, Diff, FileCode, GitCommitHorizontal, FolderTree } from 'lucide-react'
...
export default function WorkspaceView({ todoId }: Props) {
  ...
  // Commits view
  type SidebarView = 'explorer' | 'commits'
  const [sidebarView, setSidebarView] = useState<SidebarView>('explorer')
  const [commitsList, setCommitsList] = useState<Array<{ hash: string; author: string; date: string; message: string; files_changed: number; files: Array<{ status: string; path: string }> }>>([])
  ...
  const loadCommits = useCallback(async () => {
    ...
    try {
      const c = await todos.workspace.commits(todoId, selectedRepo)
      setCommitsList(c)
    } catch {
      setCommitsList([])
    } finally {
  ...
  const handleViewCommit = useCallback(async (hash: string) => {
    ...
    try {
      const detail = await todos.workspace.commitDetail(todoId, hash, selectedRepo)
      setFullDiff({ diff: detail.diff, stats: detail.stats })
    } catch {
      setFullDiff({ diff: '', stats: 'Failed to load commit diff' })
    } finally {
  ...
  // Load commits when switching to commits view
  useEffect(() => {
    if (sidebarView === 'commits' && commitsList.length === 0) {
      loadCommits()
    }
  ...
  return (
    <div className="flex h-full overflow-hidden bg-gray-950">
      ...
      >
        ...
        <div className="flex items-center justify-between px-1.5 py-1 border-b border-gray-800">
          <div className="flex items-center gap-0.5">
            <button
              onClick={() => setSidebarView('explorer')}
              className={`p-1.5 rounded transition-colors ${sidebarView === 'explorer' ? 'text-white bg-gray-800' : 'text-gray-600 hover:text-gray-400'}`}
              title="Explorer"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded transition-colors ${sidebarView === 'commits' ? 'text-white bg-gray-800' : 'text-gray-600 hover:text-gray-400'}`}
              title="Commits"
            >
        ...
        {sidebarView === 'commits' && (
          <div className="flex-1 overflow-y-auto">
            ...
            {!commitsLoading && commitsList.map((commit) => (
              <button
                key={commit.hash}
                onClick={() => handleViewCommit(commit.hash)}
                className={`w-full text-left px-3 py-2 border-b border-gray-800/50 hover:bg-gray-900/50 transition-colors ${
                  selectedCommit === commit.hash ? 'bg-gray-900 border-l-2 border-l-indigo-500' : ''
                }`}
              >
                <div className="text-xs text-gray-300 leading-snug truncate">{commit.message}</div>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[10px] text-gray-600 font-mono">{commit.hash.slice(0, 7)}</span>
                  <span className="text-[10px] text-gray-700">{commit.files_changed} file{commit.files_changed !== 1 ? 's' : ''}</span>
                  <span className="text-[10px] text-gray-700 ml-auto">{new Date(commit.date).toLocaleDateString()}</span>
                </div>
              </button>
            ))}
            <button
              onClick={loadCommits}
              className="w-full px-3 py-2 text-[11px] text-indigo-400 hover:text-indigo-300 transition-colors"
            >
              Refresh
            </button>
          </div>
